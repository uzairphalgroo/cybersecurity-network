    ...
    remediation_patches: isHighRisk ? [
      {
        ...
        remediated_terraform_hcl: `# Sentinara Remediated Patch for ${envMeta.name}
resource "aws_iam_policy" "remediated_${envMeta.id.replace(/-/g, '_')}" {
  name        = "Sentinara_Scoped_${envMeta.id.replace(/-/g, '_')}"
  ...
  users.forEach((u: any, idx: number) => {
    ...
    if (u.mfa_enabled === false) {
      ...
      remediationPatches.push({
        ...
        remediated_terraform_hcl: `# Sentinara Remediated Patch: Enforce MFA Policy
    ...
    policies.forEach((p: any) => {
      ...
      if (isWildcard || isPassRole) {
        ...
        remediationPatches.push({
          ...
          remediated_terraform_hcl: `# Sentinara Remediated Patch: Least Privilege Policy
resource "aws_iam_policy" "scoped_policy_${uname.replace(/[^a-zA-Z0-9_]/g, '_')}" {
  name        = "Sentinara_Scoped_${uname.replace(/[^a-zA-Z0-9_]/g, '_')}"
  description = "Remediated least privilege policy generated by Sentinara"
  ...
  buckets.forEach((b: any, idx: number) => {
    ...
    if (isPublic) {
      ...
      remediationPatches.push({
        ...
        remediated_terraform_hcl: `# Sentinara Remediated Patch: S3 Block Public Access
  ...
  restrict_public_buckets = true
    ...
    if (isUnencrypted) {
      ...
      remediationPatches.push({
        ...
        remediated_terraform_hcl: `# Sentinara Remediated Patch: Default Encryption
    ...
    rules.forEach((r: any, rIdx: number) => {
      ...
      if (isOpen && (fromPort === 22 || toPort === 22 || fromPort === 3306 || toPort === 3306 || fromPort === 5432 || toPort === 5432 || fromPort === 0)) {
        ...
        remediationPatches.push({
          ...
          remediated_terraform_hcl: `# Sentinara Remediated Patch: Restrict Security Group
  ...
  k8sBindings.forEach((b: any, idx: number) => {
    ...
    if (isClusterAdmin) {
      ...
      remediationPatches.push({
        ...
        remediated_terraform_hcl: `# Sentinara Remediated Patch: Scoped Kubernetes Role
  ...
  return {
    ...
    timeline: [
      ...
      },
      {
        ...
        epoch_id: 'T3',
        timestamp: 'Projected State (Sentinara Remediated)',
